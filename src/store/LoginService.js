...
import {BASE_URL,HEADERS } from "../utils/UrlUtils";


export function validateLogin(email, pwd) {
    const URI = "/api/v1/auth/authenticate";
    // const BASE_URL = "/admin/validateLogin";
    const data = {
        "email": email,
        "password": pwd
    }
...
export const activateAccount = (userId) => {
    return axios.get(BASE_URL + '/admin/admin_activateUser/' + userId);
}

export const processForgotPassword = (email) => {
    return axios.get(BASE_URL + '/admin/admin_sendForgotPasswordEmail/' + email);
}

export const setNewPassword = (data) => {
    return axios.post(BASE_URL + '/admin/admin_setNewPassword', data);
}
